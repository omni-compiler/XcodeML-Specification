...
The {\tt FbasicType} element defines a reference to prmitive types or other type definition elements.
...
{Specifies the length of the character string. Can be specified only when the type attribute of the type is Fcharacter.}{O}
...
{\tt 0}(or {\tt false}) is specifies if the type element is the function or subroutine depending on the type of the return\_type attribute.}{O}
...
By the restriction of Fortran 90, the is\_internal\_private attribute cannot be specified if is\_sequence is {\tt 0}(or {\tt false})}{O}
...
%((exprModel | len)+)
%}
\XcodeMLContentsModel{ ((exprModel | len)+) }
...
{Specifies the final subroutine.}{O}
\end{XcodeMLChildElements}
...
The {\tt finalProcedure} element defines a final subroutine.
...
\begin{XcodeMLChildElements}
\XcodeMLElementDef{name}
{Specifies the name of the final subroutine.}{R}
\end{XcodeMLChildElements}
